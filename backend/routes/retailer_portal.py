...
async def attach_credit_notes_to_rejections(rejections: list) -> list:
    ...
    # Bulk lookup credit notes for these rejections - include adjusted_in_invoices array
    credit_notes = await db.retailer_credit_notes.find(
        {"rejection_id": {"$in": rejection_ids}},
        {"_id": 0, "rejection_id": 1, "id": 1, "credit_note_number": 1, "amount": 1, "status": 1, "adjusted_in_invoices": 1, "adjusted_against_invoices": 1}
    ...
    for cn in credit_notes:
        ...
        # Get the invoice numbers from adjusted_in_invoices (primary) or adjusted_against_invoices (fallback)
        adjusted_invoices = cn.get("adjusted_in_invoices") or cn.get("adjusted_against_invoices", [])
...
async def generate_single_auto_indent(
    ...
    try:
        ...
        generation_label = f"Auto Generated by {generated_by}. Sales Based"
        response = {
            "success": True,
            "message": f"Auto indent created for {retailer_name} with {len(indent_items)} products ({generation_label})",
            "retailer_name": retailer_name,
...
async def get_all_retailers_immediately_payable(
    ...
    for inv in invoices:
        ...
        if is_full_upfront:
            ...
            if days_since == 0:
                # Today's invoice: upfront % is due; credit-period half is outstanding but not yet due
                if unpaid_upfront > 0:
                ...
                # Upfront portion overdue; credit-period half outstanding but not yet due (< day 5)
                if unpaid_upfront > 0:
