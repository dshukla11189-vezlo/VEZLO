...
# Uses Weighted Moving Average + Day-of-week patterns + Trend analysis
# ============================================================================

@app.get("/api/reports/demand-forecast")
async def get_demand_forecast(
    current_user: dict = Depends(get_current_user)
):
    """
    AI-powered demand forecasting for tomorrow's Retail orders.
    
    Logic:
    1. Weighted Moving Average (last 14 days, recent days weighted higher)
    2. Day-of-week patterns (same day last 2 weeks)
    3. Growth/decline trend analysis
    4. Product-wise demand per retailer
    """
    if current_user["role"] not in ["admin", "staff"]:
        raise HTTPException(status_code=403, detail="Not authorized")
    ...
    date_14_days_ago = (today - timedelta(days=14)).strftime('%Y-%m-%d')
    today_str = today.strftime('%Y-%m-%d')
    tomorrow_str = tomorrow.strftime('%Y-%m-%d')
    
    # Fetch retail dispatches for last 14 days (no status filter as dispatches may not have status)
    dispatches = await db.retailer_dispatches.find({
        "dispatch_date": {"$gte": date_14_days_ago, "$lte": today_str}
    }, {"_id": 0}).to_list(5000)
    
    # Fetch retailer info
    retailers = await db.retailers.find({}, {"_id": 0, "id": 1, "name": 1, "company_name": 1}).to_list(100)
    retailer_map = {r["id"]: r.get("company_name") or r.get("name", "Unknown") for r in retailers}
    
    # Fetch product info for translations
    products = await db.products.find({}, {"_id": 0, "id": 1, "name": 1, "name_hi": 1}).to_list(500)
    product_map = {p["id"]: p for p in products}
    product_name_map = {p["name"]: p for p in products}
    
    # Group data by retailer, date, and product
    retailer_daily_sales = defaultdict(lambda: defaultdict(lambda: {"sales": 0, "qty": 0, "products": defaultdict(lambda: {"qty": 0, "value": 0})}))
    
    for dispatch in dispatches:
        retailer_id = dispatch.get("retailer_id", "")
        retailer_name = retailer_map.get(retailer_id, dispatch.get("retailer_name", "Unknown"))
        dispatch_date = dispatch.get("dispatch_date", "")[:10]
        
        for item in dispatch.get("items", []):
            qty = item.get("supplied_qty", 0) or item.get("dispatched_qty", 0) or 0
            value = item.get("total_value", 0) or item.get("mrp_value", 0) or (qty * (item.get("mrp", 0) or 0))
            product_name = item.get("product_name", "Unknown")
            
            retailer_daily_sales[retailer_name][dispatch_date]["sales"] += value
            retailer_daily_sales[retailer_name][dispatch_date]["qty"] += qty
            retailer_daily_sales[retailer_name][dispatch_date]["products"][product_name]["qty"] += qty
            retailer_daily_sales[retailer_name][dispatch_date]["products"][product_name]["value"] += value
    
    # Calculate forecasts per retailer
    retailer_forecasts = []
    
    for retailer_name, daily_data in retailer_daily_sales.items():
        if not daily_data:
            continue
        ...
        for date_str in dates_with_data:
            ...
            days_ago = (today - date_obj.replace(tzinfo=timezone.utc)).days
            
            sales = daily_data[date_str]["sales"]
            qty = daily_data[date_str]["qty"]
            
            all_sales.append(sales)
            all_qty.append(qty)
            
            # Same weekday as tomorrow
            if date_obj.weekday() == tomorrow_weekday:
                same_weekday_sales.append(sales)
                same_weekday_qty.append(qty)
            
            # Recent 7 days (weighted more heavily)
            if days_ago <= 7:
                recent_7_day_sales.append(sales)
                recent_7_day_qty.append(qty)
        
        if not all_sales:
            continue
        
        # FORECASTING LOGIC
        # 1. Base: Simple average of all data
        avg_sales = statistics.mean(all_sales) if all_sales else 0
        avg_qty = statistics.mean(all_qty) if all_qty else 0
        
        # 2. Day-of-week adjustment
        dow_sales = statistics.mean(same_weekday_sales) if same_weekday_sales else avg_sales
        dow_qty = statistics.mean(same_weekday_qty) if same_weekday_qty else avg_qty
        
        # 3. Recent trend (last 7 days weighted 60%, day-of-week weighted 40%)
        recent_avg_sales = statistics.mean(recent_7_day_sales) if recent_7_day_sales else avg_sales
        recent_avg_qty = statistics.mean(recent_7_day_qty) if recent_7_day_qty else avg_qty
        
        # 4. Trend analysis (compare recent 7 days to previous 7 days)
        if len(all_sales) >= 7:
            older_sales = all_sales[:-7] if len(all_sales) > 7 else all_sales[:len(all_sales)//2]
            older_avg = statistics.mean(older_sales) if older_sales else avg_sales
            trend_multiplier = (recent_avg_sales / older_avg) if older_avg > 0 else 1.0
            trend_multiplier = max(0.7, min(1.3, trend_multiplier))  # Cap at ±30%
        else:
            trend_multiplier = 1.0
        
        # 5. Final weighted forecast
        # 50% recent average + 30% day-of-week + 20% overall average, then apply trend
        forecast_sales = (0.5 * recent_avg_sales + 0.3 * dow_sales + 0.2 * avg_sales) * trend_multiplier
        forecast_qty = (0.5 * recent_avg_qty + 0.3 * dow_qty + 0.2 * avg_qty) * trend_multiplier
        
        # Calculate confidence based on data consistency
        if len(all_sales) >= 7:
            cv = (statistics.stdev(all_sales) / avg_sales * 100) if avg_sales > 0 else 100
            confidence = "High" if cv < 30 else "Medium" if cv < 60 else "Low"
        else:
            confidence = "Low"
        
        # Product-wise forecast
        product_forecasts = []
        product_totals = defaultdict(lambda: {"total_qty": 0, "total_value": 0, "occurrences": 0, "recent_qty": 0, "dow_qty": 0})
        
        for date_str, day_data in daily_data.items():
            date_obj = datetime.strptime(date_str, '%Y-%m-%d')
            days_ago = (today - date_obj.replace(tzinfo=timezone.utc)).days
            
            for product_name, pdata in day_data["products"].items():
                product_totals[product_name]["total_qty"] += pdata["qty"]
                product_totals[product_name]["total_value"] += pdata["value"]
                product_totals[product_name]["occurrences"] += 1
                
                if days_ago <= 7:
                    product_totals[product_name]["recent_qty"] += pdata["qty"]
                if date_obj.weekday() == tomorrow_weekday:
                    product_totals[product_name]["dow_qty"] += pdata["qty"]
        
        for product_name, pdata in sorted(product_totals.items(), key=lambda x: x[1]["total_qty"], reverse=True)[:10]:
            avg_product_qty = pdata["total_qty"] / max(pdata["occurrences"], 1)
            recent_product_qty = pdata["recent_qty"] / min(7, len(dates_with_data)) if pdata["recent_qty"] > 0 else avg_product_qty
            dow_product_qty = pdata["dow_qty"] / max(1, len(same_weekday_sales)) if pdata["dow_qty"] > 0 else avg_product_qty
            
            forecast_product_qty = (0.5 * recent_product_qty + 0.3 * dow_product_qty + 0.2 * avg_product_qty) * trend_multiplier
            
            # Get product translation
            prod_info = product_name_map.get(product_name, {})
            
            product_forecasts.append({
                "product": product_name,
                "product_hi": prod_info.get("name_hi", ""),
                "forecast_qty": round(forecast_product_qty, 1),
                "avg_qty": round(avg_product_qty, 1),
                "recent_avg": round(recent_product_qty, 1)
            })
        
        # Trend description
        if trend_multiplier > 1.1:
            trend_desc = f"↑ Growing ({((trend_multiplier-1)*100):.0f}% up)"
        elif trend_multiplier < 0.9:
            trend_desc = f"↓ Declining ({((1-trend_multiplier)*100):.0f}% down)"
        else:
            trend_desc = "→ Stable"
        
        retailer_forecasts.append({
            "retailer": retailer_name,
            "forecast_date": tomorrow_str,
            "forecast_day": weekday_names[tomorrow_weekday],
            "forecast_sales": round(forecast_sales, 2),
            "forecast_qty": round(forecast_qty, 1),
            "avg_daily_sales": round(avg_sales, 2),
            "avg_daily_qty": round(avg_qty, 1),
            "same_day_avg_sales": round(dow_sales, 2),
            "same_day_avg_qty": round(dow_qty, 1),
            "trend": trend_desc,
            "trend_multiplier": round(trend_multiplier, 2),
            "confidence": confidence,
            "data_days": len(dates_with_data),
            "products": product_forecasts[:10]  # Top 10 products
        })
    
    # Sort by forecast sales
    retailer_forecasts.sort(key=lambda x: x["forecast_sales"], reverse=True)
    
    # Calculate totals
    total_forecast_sales = sum(r["forecast_sales"] for r in retailer_forecasts)
    total_forecast_qty = sum(r["forecast_qty"] for r in retailer_forecasts)
    ...
    return {
        ...
        "methodology": {
            "name": "Weighted Moving Average + Day-of-Week + Trend",
            "description": "Combines 50% recent 7-day average, 30% same-weekday average, 20% overall average, with trend adjustment",
            "data_period": "Last 14 days",
            "factors": [
                "Recent sales patterns (last 7 days)",
                f"Day-of-week patterns ({weekday_names[tomorrow_weekday]} history)",
                "Growth/decline trend analysis"
            ]
        },
        "summary": {
            "total_retailers": len(retailer_forecasts),
            "total_forecast_sales": round(total_forecast_sales, 2),
            "total_forecast_qty": round(total_forecast_qty, 1)
        },
        "retailers": retailer_forecasts
    }
